#!/usr/bin/env python3
"""CLI helper to sanity-check PettingZoo MPE environments."""
...
from mpe_env_setup.env_factory import PRESET_SPECS
from mpe_env_setup.rollout import run_random_episode
...
def main() -> None:
    ...
    output_path = Path.cwd() / "rollout_summary.json"
    output_path.write_text(json.dumps(summaries, indent=2 if args.pretty else None))
    print(f"Wrote {len(summaries)} summary entries to {output_path}")
